...
class DialogueCharacter extends FlxSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, character:String = null)
	{
		...
		frames = backend.Paths.getSparrowAtlas('dialogue/' + jsonFile.image);
		reloadAnimations();
...
{
	var dialogue:object.TypedAlphabet;
	...
	public function new(dialogueList:DialogueFile, ?song:String = null)
	{
		...
		if(song != null && song != '') {
			FlxG.sound.playMusic(backend.Paths.music(song), 0);
	...
	var daText:object.TypedAlphabet = null;
	...
	override function update(elapsed:Float)
	{
		...
		if(!dialogueEnded) {
			...
			if(PlayerSettings.player1.controls.ACCEPT) {
				...
				FlxG.sound.play(backend.Paths.sound(closeSound), closeVolume);
