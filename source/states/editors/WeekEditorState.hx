...
class WeekEditorState extends states.MusicBeatState
{
	...
	override function create() {
		...
		missingFileText.setFormat(backend.Paths.uiFont, 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y + 435).loadGraphic(backend.Paths.image('Menu_Tracks'));
		tracksSprite.antialiasing = ClientPrefs.globalAntialiasing;
		add(tracksSprite);

		txtTracklist = new FlxText(FlxG.width * 0.05, tracksSprite.y + 60, 0, "", 32);
		txtTracklist.alignment = CENTER;
		txtTracklist.font = backend.Paths.uiFont;
	...
	function addEditorBox() {
		...
		var freeplayButton:FlxButton = new FlxButton(0, 650, "Freeplay", function() {
			states.MusicBeatState.switchState(new WeekEditorFreeplayState(weekFile));
	...
	function reloadBG() {
		...
		if(assetName != null && assetName.length > 0) {
			if(Assets.exists(backend.Paths.getPath('images/menubackgrounds/menu_' + assetName + '.png', IMAGE), IMAGE)) {
				bgSprite.loadGraphic(backend.Paths.image('menubackgrounds/menu_' + assetName));
				isMissing = false;
			}
		}
	...
	function reloadWeekThing() {
		...
		var isMissing:Bool = true;
		if(assetName != null && assetName.length > 0) {
			if(Assets.exists(backend.Paths.getPath('images/storymenu/' + assetName + '.png', IMAGE), IMAGE)) {
				weekThing.loadGraphic(backend.Paths.image('storymenu/' + assetName));
				isMissing = false;
			}
		}
...
class WeekEditorFreeplayState extends states.MusicBeatState
{
	var weekFile:WeekFile = null;
	public function new(weekFile:WeekFile = null)
	{
	...
	override function update(elapsed:Float) {
		if(WeekEditorState.loadedWeek != null) {
			...
			states.MusicBeatState.switchState(new WeekEditorFreeplayState(WeekEditorState.loadedWeek));
