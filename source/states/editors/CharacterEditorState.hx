...
class CharacterEditorState extends states.MusicBeatState
{
	...
	var bgCharStuff:BGCharacter;

	override function create()
	{
		//FlxG.sound.playMusic(backend.Paths.music('breakfast'), 0.5);
		...
		healthBarBG = new FlxSprite(30, FlxG.height - 75).loadGraphic(backend.Paths.image('healthBar'));
		...
		textAnim.setFormat(backend.Paths.uiFont, 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var tabs = [
			...
			{name: 'BG Char',	label:' Bg Character'}
		];
	...
	function reloadBGs() {
		...
		if(onPixelBG) {
			...
			bgTrees.frames = backend.Paths.getPackerAtlas('weeb/weebTrees');
			...
			var bg:BGSprite = new BGSprite('bg/stage/stageback', -600 + OFFSET_X - playerXDifference, -300, 0.9, 0.9);
			bgLayer.add(bg);

			var stageFront:BGSprite = new BGSprite('bg/stage/stagefront', -650 + OFFSET_X - playerXDifference, 500, 0.9, 0.9);
	...
	var stepper_COMBO:FlxUINumericStepper;
	var checkbox_LOOK:FlxUICheckBox;
	var checkbox_CHEER:FlxUICheckBox;
	function addBGCharUI() {
		var tabGroup = new FlxUI(null, UI_box);
		tabGroup.name = 'BG Char';

		if (char.bgData == null) {
			char.bgData = {lookAtSinger: false,
			cheerOnChain: false,
		cheerLimit: 10};
		}

		checkbox_LOOK = new FlxUICheckBox(15, 25, null, null, 'Looks at the signer?');
		checkbox_LOOK.checked = char.bgData.lookAtSinger;
		checkbox_LOOK.callback = () -> {
			char.bgData.lookAtSinger = checkbox_LOOK.checked;
		}

		checkbox_CHEER = new FlxUICheckBox(15, 70, null, null, 'Cheer on combo?');
		checkbox_CHEER.checked = char.bgData.cheerOnChain;
		checkbox_CHEER.callback = () -> {
			char.bgData.cheerOnChain = checkbox_CHEER.checked;
		}

		stepper_COMBO = new FlxUINumericStepper(135, 70, 1, char.bgData.cheerLimit, 0, 999, 0);

		tabGroup.add(checkbox_LOOK);
		tabGroup.add(checkbox_CHEER);
		tabGroup.add(stepper_COMBO);
		tabGroup.add(new FlxText(135, stepper_COMBO.y - 18, 0, 'How many combos to cheer at:'));
		UI_characterbox.addGroup(tabGroup);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			...
			else if(sender == healthColorStepperB)
			{
				...
				healthBarBG.color = FlxColor.fromRGB(char.healthColorArray[0], char.healthColorArray[1], char.healthColorArray[2]);
			} else if(sender == stepper_COMBO) {
				char.bgData.cheerLimit = Std.int(stepper_COMBO.value);
			}
		}
	}
	...
	function reloadCharacterImage() {
		...
		if(backend.Paths.fileExists('images/' + char.imageFile + '/Animation.json', TEXT)) {
			char.frames = AtlasFrameMaker.construct(char.imageFile);
		} else if(backend.Paths.fileExists('images/' + char.imageFile + '.txt', TEXT)) {
			char.frames = backend.Paths.getPackerAtlas(char.imageFile);
		} else {
			char.frames = backend.Paths.getSparrowAtlas(char.imageFile);
		}
	...
	function reloadCharacterOptions() {
		if(UI_characterbox != null) {
			...
			checkbox_LOOK.checked = char.bgData.lookAtSinger;
			checkbox_CHEER.checked = char.bgData.cheerOnChain;
			stepper_COMBO.value = Std.int(char.bgData.cheerLimit);
			updatePresence();
		}
	}
	...
	function reloadCharacterDropDown() {
		...
		//characterList = CoolUtil.coolTextFile(backend.Paths.txt('characterList'));

		characterList = [];
		for (i in FileSystem.readDirectory('assets/characters')) {
			characterList.push(i.replace('.json',''));
		}
	...
	{
		states.MusicBeatState.camBeat = FlxG.camera;
		if(char.animationsArray[curAnim] != null) {
