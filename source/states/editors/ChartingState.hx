...
import object.Character.CharacterFile;
...
class ChartingState extends states.MusicBeatState
{
	...
	**/
	...
	public var normalEventsLast:Bool = false;
	...
	var currentCutscenePath:String = '';
	var currentCutsceneExtension:String = '';
	var currentCoverType:String = '';

	var previewDat:Array<String> = [null, null];

	var coverSprite:FlxUISprite;
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(backend.Paths.image('menuDesat'));
		bg.scrollFactor.set();
		bg.color = 0xFF222222;
		add(bg);
		...
		var eventIcon:FlxSprite = new FlxSprite(-GRID_SIZE - 5, -90).loadGraphic(backend.Paths.image('eventArrow'));
		...
		currentSongName = backend.Paths.formatToSongPath(_song.song);
		if (_song.cutscene == null) {
			_song.cutscene = {
				hasCutscene: false,
				filePath: 'cutscenes/demo',
				fileExtension: 'mp4',
		...
		currentCutscenePath = _song.cutscene.filePath;
		currentCutsceneExtension = _song.cutscene.fileExtension;
		loadSong();
		reloadGridLayer();
		...
		for (i in 0...tipTextArray.length) {
			...
			tipText.y += i * 12;
			tipText.setFormat(backend.Paths.uiFont, 14, FlxColor.WHITE, LEFT/*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
		...
		//coverSprite = new FlxSprite().loadGraphic(backend.Paths.image('FileNotFound'));
		coverSprite = new FlxUISprite();
		coverSprite.loadGraphic(backend.Paths.image('FileNotFound'));
	...
	var UI_cutsceneExtension:FlxUIInputText;
	var UI_cutscenePath:FlxUIInputText;
	var UI_coverType:FlxUIInputText;
	var UI_coverVal:FlxUIInputText;
	function addCutsceneUI() {
		var check_cutscene = new FlxUICheckBox(10, 15, null, null, "Has cutscene?", 100);
		check_cutscene.checked = _song.cutscene.hasCutscene;
		check_cutscene.callback = function () {
			_song.cutscene.hasCutscene = check_cutscene.checked;
		}

		UI_cutscenePath = new FlxUIInputText(120, 15, 100, _song.cutscene.filePath, 8);
		blockPressWhileTypingOn.push(UI_cutscenePath);

		UI_cutsceneExtension = new FlxUIInputText(230, 15, 25, _song.cutscene.fileExtension, 8);
		blockPressWhileTypingOn.push(UI_cutsceneExtension);

		var fadeOutTime = new FlxUINumericStepper(10, 50, 0.1, _song.cutscene.fadeOutTime, 0, 999, 1);
		fadeOutTime.value = _song.cutscene.fadeOutTime;
		fadeOutTime.name = 'song_cutscene_fadeOutTime';
		blockPressWhileTypingOnStepper.push(fadeOutTime);

		var pauseBegin = new FlxUINumericStepper(fadeOutTime.x + 65, fadeOutTime.y, 0.25,  _song.cutscene.beginningDelay, 0, 999, 2);
		pauseBegin.value = _song.cutscene.beginningDelay;
		pauseBegin.name = 'song_cutscene_beginningDelay';
		blockPressWhileTypingOnStepper.push(pauseBegin);

		UI_coverType = new FlxUIInputText(10, 85, 75, _song.cutscene.coverType);
		blockPressWhileTypingOn.push(UI_coverType);

		var check_constantCover = new FlxUICheckBox(95, 85, null, null, 'Constant cover', 50);
		check_constantCover.checked = _song.cutscene.constantCover;
		check_constantCover.callback = function() {
			_song.cutscene.constantCover = check_constantCover.checked;
		}
		check_constantCover.textY -= 10;

		UI_coverVal = new FlxUIInputText(170, 85, 75, _song.cutscene.coverVal, 8);
		blockPressWhileTypingOn.push(UI_coverVal);

		var coverScaleStepper = new FlxUINumericStepper(10, 115, 0.05, _song.cutscene.coverScale[0], 0, 10, 2);
		coverScaleStepper.value = _song.cutscene.coverScale[0];
		coverScaleStepper.name = 'song_cutscene_scale';
		blockPressWhileTypingOnStepper.push(coverScaleStepper);

		var canSkip = new FlxUICheckBox(10, 225, null, null, 'Can skip?', 75);
		canSkip.checked = _song.cutscene.canSkip;
		canSkip.callback = function() {
			_song.cutscene.canSkip=canSkip.checked;
		}

		var fadeOnSkip = new FlxUICheckBox(100, 225, null, null, 'Fade on skip?', 85);
		fadeOnSkip.checked = _song.cutscene.fadeOnSkip;
		fadeOnSkip.callback = function() {
			_song.cutscene.fadeOnSkip=fadeOnSkip.checked;
		}

		var alwayPlay = new FlxUICheckBox(200, 225, null, null, 'Always play cutscene?', 90);
		alwayPlay.checked = _song.cutscene.alwaysPlay;
		alwayPlay.callback = function() {
			_song.cutscene.alwaysPlay=alwayPlay.checked;
		}
		#if sys
		var loadCoverButton = new FlxUIButton(coverScaleStepper.x + 75, coverScaleStepper.y-3, 'Load cover', () -> {
		...
		#else
		var loadCoverButton = new FlxUIButton(coverScaleStepper.x + 75, coverScaleStepper.y-3, 'cover button not supported on HTML5 :(');
		#end


		var tab_group_cutscene = new FlxUI(null, UI_box);
		tab_group_cutscene.name = 'Cutscene';

		coverSprite.visible = false;
		reloadCoverSprite();

		tab_group_cutscene.add(coverSprite);
		tab_group_cutscene.add(check_cutscene);
		tab_group_cutscene.add(UI_cutscenePath);
		tab_group_cutscene.add(UI_cutsceneExtension);
		tab_group_cutscene.add(fadeOutTime);
		tab_group_cutscene.add(pauseBegin);
		tab_group_cutscene.add(UI_coverType);
		tab_group_cutscene.add(check_constantCover);
		tab_group_cutscene.add(UI_coverVal);
		tab_group_cutscene.add(coverScaleStepper);
		tab_group_cutscene.add(canSkip);
		tab_group_cutscene.add(fadeOnSkip);
		tab_group_cutscene.add(alwayPlay);
		tab_group_cutscene.add(new FlxText(UI_cutscenePath.x, UI_cutscenePath.y - 12, 0, 'Cutscene Path:'));
		tab_group_cutscene.add(new FlxText(UI_cutsceneExtension.x-8, UI_cutsceneExtension.y, 0, '.'));
		tab_group_cutscene.add(new FlxText(fadeOutTime.x, fadeOutTime.y - 15, 0, 'Fade out time:'));
		tab_group_cutscene.add(new FlxText(pauseBegin.x, pauseBegin.y - 15, 0, 'Start delay:'));
		tab_group_cutscene.add(new FlxText(UI_coverType.x, UI_coverType.y - 15, 0, 'Cover type:'));
		tab_group_cutscene.add(new FlxText(UI_coverVal.x, UI_coverVal.y - 15, 0, 'Cover value:'));
		tab_group_cutscene.add(new FlxText(coverScaleStepper.x, coverScaleStepper.y - 15, 0, 'Cover scale:'));
		tab_group_cutscene.add(loadCoverButton);

		UI_box.addGroup(tab_group_cutscene);
	}
	...
	function selectedCover(e:Event) {
		...
		_song.cutscene.coverType = 'image';
		_song.cutscene.coverType = ret;
		var lib = null;
		var mini = sto.split('/');
		if (mini[2] != 'images') 
			lib = mini[2];
		...
		reloadCoverSprite();
		#else 
		trace('cover button not supported on HTML5 :(');
		#end
	}

	function reloadCoverSprite() {
		if (previewDat[0] == null) {
			previewDat[0] = _song.cutscene.coverVal;
			UI_coverType.text = _song.cutscene.coverType;
			UI_coverVal.text = _song.cutscene.coverVal;
		}
		var graphic:FlxGraphic = null;
		if (UI_coverType.text.toLowerCase() == 'color') {
			coverSprite.makeGraphic(133, 75, 0xFF000000);
		} else {
			graphic = backend.Paths.image(previewDat[0], previewDat[1]);
			if (graphic == null) {
				graphic = backend.Paths.image('FileNotFound');
			} else {
				UI_coverType.text = 'image';
				UI_coverVal.text = previewDat[0];
				if (previewDat[1] == 'shared') 
					UI_coverVal.text = previewDat[1] +'/'+previewDat[0]; 
				
			}
			coverSprite.loadGraphic(graphic);
		}
		
		coverSprite.setGraphicSize(133, 75);
		coverSprite.updateHitbox();
		coverSprite.updateFramePixels();
		coverSprite.x = 15;
		coverSprite.y =  135;
	}
	...
	function addSongUI():Void
	{
		...
		{
			currentSongName = backend.Paths.formatToSongPath(UI_songTitle.text);

			currentCutscenePath = UI_cutscenePath.name;
			currentCutsceneExtension = UI_cutsceneExtension.name;
			
			currentCoverType = UI_cutsceneExtension.name;
			loadSong();
			updateWaveform();
		});
		...
		var loadEventJson:FlxButton = new FlxButton(loadAutosaveBtn.x, loadAutosaveBtn.y + 30, 'Load Events', function()
		{
			...
			var songName:String = backend.Paths.formatToSongPath(_song.song);
		...
		var directories:Array<String> = [backend.Paths.getPreloadPath('characters/')];

		var tempMap:Map<String, Bool> = new Map<String, Bool>();
		var characters:Array<String> = [];
		for (i in FileSystem.readDirectory('assets/characters')) {
		...
		var directories:Array<String> = [backend.Paths.getPreloadPath('stages/')];

		tempMap.clear();
		var stageFile:Array<String> = [];
		for (i in FileSystem.readDirectory('assets/stages')) {
			stageFile.push(i.replace('.json',''));
		}
	...
	var shaderEventDropDown:FlxUIDropDownMenuCustom;
	...
	function addEventsUI():Void
	{
		...
		descText = new FlxText(20, 200, 0, backend.events.EventsCore.eventStuff[0][0]);

		var leEvents:Array<String> = [];
		for (i in 0...backend.events.EventsCore.eventStuff.length) {
			leEvents.push(backend.events.EventsCore.eventStuff[i][0]);
		}

		var value2displaytext:FlxText = new FlxText(20, 150, 0, "Value 2:");
		var value1DisplayText:FlxText = new FlxText(20, 110, 0, "Value 1:");

		var text:FlxText = new FlxText(20, 10, 0, "Events:");
		tab_group_event.add(text);
		eventDropDown = new FlxUIDropDownMenuCustom(20, 30, FlxUIDropDownMenuCustom.makeStrIdLabelArray(leEvents, true), function(pressed:String) {
			normalEventsLast = true;
			var selectedEvent:Int = Std.parseInt(pressed);
			value2InputText.visible = true;
			value2displaytext.visible = true;
			value1DisplayText.text = 'Value 1:';
			descText.text = backend.events.EventsCore.eventStuff[selectedEvent][1];
				if (curSelectedNote != null &&  backend.events.EventsCore.eventStuff != null) {
				if (curSelectedNote != null && curSelectedNote[2] == null){
				curSelectedNote[1][curEventSelected][0] = backend.events.EventsCore.eventStuff[selectedEvent][0];
				}
				updateGrid();
			}
		});
		...
		leEvents = [];
		for (i in 0...backend.events.EventsCore.shaderEvents.length) {
			leEvents.push(backend.events.EventsCore.shaderEvents[i][0]);
		}

		var text:FlxText = new FlxText(20, 60, 0, "Shader Events:");
		tab_group_event.add(text);
		shaderEventDropDown = new FlxUIDropDownMenuCustom(20, 80, FlxUIDropDownMenuCustom.makeStrIdLabelArray(leEvents, true), function(pressed:String) {
			normalEventsLast = false;
			var selectedEvent:Int = Std.parseInt(pressed);
			value2InputText.visible = false;
			value2displaytext.visible = false;
			value1DisplayText.text = 'Shader Options';
			descText.text = backend.events.EventsCore.shaderEvents[selectedEvent][1];
				if (curSelectedNote != null &&  backend.events.EventsCore.shaderEvents != null) {
				if (curSelectedNote != null && curSelectedNote[2] == null){
				curSelectedNote[1][curEventSelected][0] = backend.events.EventsCore.shaderEvents[selectedEvent][0];
				}
				updateGrid();
			}
		});
		blockPressWhileScrolling.push(shaderEventDropDown);
		

		tab_group_event.add(value1DisplayText);
		value1InputText = new FlxUIInputText(20, 130, 100, "");
		blockPressWhileTypingOn.push(value1InputText);

		
		tab_group_event.add(value2displaytext);
		value2InputText = new FlxUIInputText(20, 170, 100, "");
		...
		tab_group_event.add(shaderEventDropDown);
	...
	function loadSong():Void
	{
		...
		var file:Dynamic = backend.Paths.voices(currentSongName);
		vocals = new FlxSound();
	...
	function generateSong() {
		FlxG.sound.playMusic(backend.Paths.inst(currentSongName), 0.6/*, false*/);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			} else if (wname == 'song_cutscene_fadeOutTime') {
				_song.cutscene.fadeOutTime = nums.value;
			} else if (wname == 'song_cutscene_beginningDelay') {
				_song.cutscene.beginningDelay = nums.value;
			} else if (wname == 'song_cutscene_scale') {
				_song.cutscene.coverScale = [nums.value, nums.value]; // trol
			}
		}
	...
	override function update(elapsed:Float)
	{
		...
		_song.cutscene.fileExtension = UI_cutsceneExtension.text;
		_song.cutscene.filePath = UI_cutscenePath.text;
		_song.cutscene.coverType = UI_coverType.text;
		_song.cutscene.coverVal = UI_coverVal.text;
		...
		if(!blockInput) {
			FlxG.sound.muteKeys = states.FirstState.muteKeys;
			FlxG.sound.volumeDownKeys = states.FirstState.volumeDownKeys;
			FlxG.sound.volumeUpKeys = states.FirstState.volumeUpKeys;
		...
		if (!blockInput)
		{
			...
			{
				autosaveSong();
				states.LoadingState.loadAndSwitchState(new states.editors.EditorPlayState(sectionStartTime()));
			}
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				states.LoadingState.loadAndSwitchState(new PlayState(), false, true);
			}
			...
			if (FlxG.keys.justPressed.BACKSPACE) {
				...
				states.MusicBeatState.switchState(new states.editors.MasterEditorMenu());
				...
				if(note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1) {
					...
					if(!playedSound[data]) {
						if((playSoundBf.checked && note.mustPress) || (playSoundDad.checked && !note.mustPress)){
							...
							FlxG.sound.play(backend.Paths.sound(soundToPlay)).pan = note.noteData < 4? -0.3 : 0.3; //would be coolio
		...
		if(metronome.checked && lastConductorPos != Conductor.songPosition) {
			...
			if(metroStep != lastMetroStep) {
				FlxG.sound.play(backend.Paths.sound('Metronome_Tick'));
				//trace('Ticked');
			}
		}
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		var path:String = backend.Paths.getPreloadPath(characterPath);
		if (!OpenFlAssets.exists(path))
		{
			path = backend.Paths.getPreloadPath('characters/' + Character.DEFAULT_CHARACTER + '.json'); //If a character couldn't be found, change him to BF just to prevent a crash
		}
		...
		if (curSelectedNote != null) {
			if(curSelectedNote[2] != null) {
				...
				if (normalEventsLast) {
					...
					if(selected > 0 && selected < backend.events.EventsCore.eventStuff.length) {
						descText.text = backend.events.EventsCore.eventStuff[selected][1];
					}
					value1InputText.text = curSelectedNote[1][curEventSelected][1];
					value2InputText.text = curSelectedNote[1][curEventSelected][2];
				} else {
					shaderEventDropDown.selectedLabel = curSelectedNote[1][curEventSelected][0];
					var selected:Int = Std.parseInt(shaderEventDropDown.selectedId);
					if(selected > 0 && selected < backend.events.EventsCore.shaderEvents.length) {
						descText.text = backend.events.EventsCore.shaderEvents[selected][1];
					}
					value1InputText.text = curSelectedNote[1][curEventSelected][1];
					value2InputText.text = curSelectedNote[1][curEventSelected][2];
				}
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(backend.Paths.uiFont, 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				daText.setFormat(backend.Paths.uiFont, 12, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE_FAST, FlxColor.BLACK);
				daText.xAdd = -410;
				daText.borderSize = 1;
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		if(daSus != null) { //Common note
			...
			note.loadGraphic(backend.Paths.image('eventArrow'));
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		{
			if (normalEventsLast) {
				var event = backend.events.EventsCore.eventStuff[Std.parseInt(eventDropDown.selectedId)][0];
				...
				var event = backend.events.EventsCore.shaderEvents[Std.parseInt(shaderEventDropDown.selectedId)][0];
				var text1 = value1InputText.text;
				var text2 = value2InputText.text;
				_song.events.push([noteStrum, [[event, text1, text2]]]);
				curSelectedNote = _song.events[_song.events.length - 1];
	...
	private function saveLevel()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), backend.Paths.formatToSongPath(_song.song) + ".json");
		}
	}
...
class ChartingState extends MusicBeatState {
	...
	public static final noteTypeList:Array<String> = //Used for backwards compatibility with 0.1 - 0.3.2 charts, though, you should add your hardcoded custom note types here too.
	[
		'',
		'Alt Animation',
		'Hey!',
	...
	override function create() {
		var bg = new FlxSprite().loadGraphic(backend.Paths.image('menuDesat'));
		bg.screenCenter();
		add(bg);
		var text = new FlxText(0,0,1100,"Chart editor not supported on Hashlink targets :(", 128);
		text.setFormat(backend.Paths.uiFont, 64, 0xFFFFFFFF, CENTER, OUTLINE, 0xFF000000);
		text.screenCenter();
		add(text);
		super.create();
	}
