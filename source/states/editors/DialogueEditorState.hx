...
class DialogueEditorState extends states.MusicBeatState
{
	...
	override function create() {
		...
		addLineText.setFormat(backend.Paths.uiFont, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		addLineText.scrollFactor.set();
		add(addLineText);

		selectedText = new FlxText(10, 32, FlxG.width - 20, '', 8);
		selectedText.setFormat(backend.Paths.uiFont, 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function reloadCharacter() {
		character.frames = backend.Paths.getSparrowAtlas('dialogue/' + character.jsonFile.image);
...
class DialogueEditorState extends MusicBeatState {
	override function create() {
		...
		bg.screenCenter();
		add(bg);
		var text = new FlxText(0,0,1100,"Dialouge editor not supported on Hashlink targets :(", 128);
