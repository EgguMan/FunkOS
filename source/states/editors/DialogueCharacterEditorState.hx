...
class DialogueCharacterEditorState extends states.MusicBeatState
{
	...
	override function create() {
		...
		box.frames = backend.Paths.getSparrowAtlas('speech_bubble');
		...
		tipText.setFormat(backend.Paths.uiFont, 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		tipText.cameras = [camHUD];
		tipText.scrollFactor.set();
		add(tipText);

		offsetLoopText = new FlxText(10, 10, 0, '', 32);
		offsetLoopText.setFormat(backend.Paths.uiFont, 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		offsetIdleText.setFormat(backend.Paths.uiFont, 32, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		animText.setFormat(backend.Paths.uiFont, 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function reloadCharacter() {
		...
		for (char in charsArray) {
			char.frames = backend.Paths.getSparrowAtlas('dialogue/' + character.jsonFile.image);
	...
	override function update(elapsed:Float) {
		...
		if(!blockInput && !animationDropDown.dropPanel.visible) {
			...
			if(FlxG.keys.justPressed.ESCAPE) {
				...
				FlxG.sound.playMusic(backend.Paths.music('freakyMenu'), 1);
				transitioning = true;
			}
...
class DialogueCharacterEditorState extends MusicBeatState {
	override function create() {
		...
		bg.screenCenter();
		add(bg);
		var text = new FlxText(0,0,1100,"Dialouge character editor not supported on Hashlink targets :(", 128);
