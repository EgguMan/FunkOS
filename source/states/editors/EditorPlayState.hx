...
class EditorPlayState extends states.MusicBeatState
{
	...
	override function create()
	{
		...
		sectionTxt.setFormat(backend.Paths.uiFont, 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		sectionTxt.scrollFactor.set();
		sectionTxt.borderSize = 1.25;
		add(sectionTxt);
		
		beatTxt = new FlxText(10, sectionTxt.y + 30, FlxG.width - 20, "Beat: 0", 20);
		beatTxt.setFormat(backend.Paths.uiFont, 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		beatTxt.scrollFactor.set();
		beatTxt.borderSize = 1.25;
		add(beatTxt);

		stepTxt = new FlxText(10, beatTxt.y + 30, FlxG.width - 20, "Step: 0", 20);
		stepTxt.setFormat(backend.Paths.uiFont, 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		stepTxt.scrollFactor.set();
		stepTxt.borderSize = 1.25;
		add(stepTxt);

		var tipText:FlxText = new FlxText(10, FlxG.height - 24, 0, 'Press ESC to Go Back to Chart Editor', 16);
		tipText.setFormat(backend.Paths.uiFont, 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	function sayGo() {
		var go:FlxSprite = new FlxSprite().loadGraphic(backend.Paths.image('go'));
		go.scrollFactor.set();
		...
		FlxG.sound.play(backend.Paths.sound('introGo'), 0.6);
	}
	...
	{
		FlxG.sound.playMusic(backend.Paths.inst(PlayState.SONG.song), 0, false);
	...
	private function endSong() {
		states.LoadingState.loadAndSwitchState(new states.editors.ChartingState());
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		rating.loadGraphic(backend.Paths.image(pixelShitPart1 + daRating + pixelShitPart2));
		rating.screenCenter();
