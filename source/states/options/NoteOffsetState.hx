...
class NoteOffsetState extends states.MusicBeatState
{
	...
	override public function create()
	{
		...
		rating = new FlxSprite().loadGraphic(backend.Paths.image('sick'));
		...
		{
			var numScore:FlxSprite = new FlxSprite(43 * daLoop).loadGraphic(backend.Paths.image('num' + i));
		...
		timeBarBG = new FlxSprite(0, timeTxt.y + 8).loadGraphic(backend.Paths.image('timeBar'));
		...
		changeModeText.setFormat(backend.Paths.uiFont, 32, FlxColor.WHITE, CENTER);
		...
		FlxG.sound.playMusic(backend.Paths.music('offsetSong'), 1, true);
	...
	override public function update(elapsed:Float)
	{
		...
		if(controls.BACK || FlxG.mouse.justReleasedRight)
		{
			...
			FlxG.sound.playMusic(backend.Paths.music('freakyMenu'), 1, true);
	...
	function createTexts()
	{
		for (i in 0...4)
		{
			...
			text.setFormat(backend.Paths.uiFont, 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
