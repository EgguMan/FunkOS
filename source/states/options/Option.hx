package states.options; // not exactly a state, but i dont think people mind it being here rather than objects. (same with the substates)
...
class Option
{
	...
	public var onInteraction:Void->Void = null;
	...
	private function get_type()
	{
		...
		{
			case 'int' | 'float' | 'percent' | 'string' | 'path': newValue = type;
			case 'integer': newValue = 'int';
			case 'str': newValue = 'string';
			case 'fl': newValue = 'float';
			case 'pt': newValue = 'path';
		}
		type = newValue;
		return type;
	}

	public function interact(i:String) {
		setValue(i);
		set_text(i);
	}
}