package states.options;

import backend.MouseHandler;
#if desktop
import backend.Discord.DiscordClient;
#end
import flash.text.TextField;
import flixel.FlxG;
import flixel.FlxSprite;
import flixel.addons.display.FlxGridOverlay;
import flixel.group.FlxGroup.FlxTypedGroup;
import flixel.math.FlxMath;
import flixel.text.FlxText;
import flixel.util.FlxColor;
import lime.utils.Assets;
import flixel.FlxSubState;
import flash.text.TextField;
import flixel.FlxG;
import flixel.FlxSprite;
import flixel.util.FlxSave;
import haxe.Json;
import flixel.tweens.FlxEase;
import flixel.tweens.FlxTween;
import flixel.util.FlxTimer;
import flixel.input.keyboard.FlxKey;
import flixel.graphics.FlxGraphic;
import backend.Controls;
import data.*;
import object.*;
import shaders.*;
import backend.save.ClientPrefs;

using StringTools;

class OptionsState extends states.MusicBeatState
{
	var options:Array<String> = ['Note Colors', 'Controls', 'Adjust Delay and Combo', 'Graphics', 'Visuals and UI', 'Gameplay', 'Accessibility'];
	private var grpOptions:FlxTypedGroup<Alphabet>;
	private static var curSelected:Int = 0;
	public static var menuBG:FlxSprite;

	function openSelectedSubstate(label:String) {
		switch(label) {
			case 'Note Colors':
				openSubState(new states.options.NotesSubState());
			case 'Controls':
				openSubState(new states.options.ControlsSubState());
			case 'Graphics':
				openSubState(new states.options.GraphicsSettingsSubState());
			case 'Visuals and UI':
				openSubState(new states.options.VisualsUISubState());
			case 'Gameplay':
				openSubState(new states.options.GameplaySettingsSubState());
			case 'Adjust Delay and Combo':
				states.LoadingState.loadAndSwitchState(new states.options.NoteOffsetState());
			case 'Accessibility':
				openSubState(new AccessibilitySettingsSubstate());
		}
	}

	var selectorLeft:Alphabet;
	var selectorRight:Alphabet;

	override function create() {
		ClientPrefs.beenSettings = true;
		#if desktop
		DiscordClient.changePresence("Options Menu", null);
		#end

		#if AeroMouse
		MouseHandler.setGraphic('idle');
		#end

		var bg:FlxSprite = new FlxSprite().loadGraphic(backend.Paths.image('menuDesat'));
		bg.color = 0xFFea71fd;
		bg.updateHitbox();

		bg.screenCenter();
		bg.antialiasing = ClientPrefs.globalAntialiasing;
		add(bg);

		grpOptions = new FlxTypedGroup<Alphabet>();
		add(grpOptions);

		for (i in 0...options.length)
		{
			var optionText:Alphabet = new Alphabet(0, 0, options[i], true);
			optionText.screenCenter();
			optionText.y += (100 * (i - (options.length / 2))) + 50;
			grpOptions.add(optionText);
		}

		selectorLeft = new Alphabet(0, 0, '>', true);
		add(selectorLeft);
		selectorRight = new Alphabet(0, 0, '<', true);
		add(selectorRight);

		changeSelection();
		ClientPrefs.saveSettings();

		super.create();
	}

	override function closeSubState() {
		super.closeSubState();
		ClientPrefs.saveSettings();
	}

	override function update(elapsed:Float) {
		super.update(elapsed);

		if (FlxG.mouse.wheel != 0) {
			changeSelection(FlxG.mouse.wheel*-1);
		}

		if (controls.UI_UP_P) {
			changeSelection(-1);
		}
		if (controls.UI_DOWN_P) {
			changeSelection(1);
		}

		if (controls.BACK || FlxG.mouse.justReleasedRight) {
			ClientPrefs.saveSettings(); //I HATE  HOW THERES LITTLE TO NO WAY TO SAVE ON COMMAND GRAAHHHH
			FlxG.sound.play(backend.Paths.sound('cancelMenu'));
			states.MusicBeatState.switchState(new states.MainMenuState());
		}

		if (controls.ACCEPT || backend.MouseHandler.justReleasedAccurately) {
			openSelectedSubstate(options[curSelected]);
		}
	}
	
	function changeSelection(change:Int = 0) {
		curSelected += change;
		if (curSelected < 0)
			curSelected = options.length - 1;
		if (curSelected >= options.length)
			curSelected = 0;

		var bullShit:Int = 0;

		for (item in grpOptions.members) {
			item.targetY = bullShit - curSelected;
			bullShit++;

			item.alpha = 0.6;
			if (item.targetY == 0) {
				item.alpha = 1;
				selectorLeft.x = item.x - 63;
				selectorLeft.y = item.y;
				selectorRight.x = item.x + item.width + 15;
				selectorRight.y = item.y;
			}
		}
		FlxG.sound.play(backend.Paths.sound('scrollMenu'));
	}
}