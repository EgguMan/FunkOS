...
class NotesSubState extends states.substates.MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			note.frames = backend.Paths.getSparrowAtlas('noteAssets/NOTE_assets');
	...
	override function update(elapsed:Float) {
		if(changingNote) {
			...
			if ((controls.ACCEPT || FlxG.mouse.justReleased) && nextAccept <= 0) {
				FlxG.sound.play(backend.Paths.sound('scrollMenu'));
				changingNote = true;
				holdTime = 0;
		...
		if ((controls.BACK || FlxG.mouse.justReleasedRight) || (changingNote && controls.ACCEPT)) {
			if(!changingNote) {
				close();
			} else {
				changeSelection();
