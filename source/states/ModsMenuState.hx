...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		noModsTxt.setFormat(backend.Paths.uiFont, 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(FileSystem.exists(path))
		{
			...
			for (i in 0...leMods.length)
			{
				if(leMods.length > 1 && leMods[0].length > 0) {
					...
					if(!backend.Paths.ignoreModFolders.contains(modSplit[0].toLowerCase()))
					{
		...
		if (FileSystem.exists("modsList.txt")){
			for (folder in backend.Paths.getModDirectories())
			{
				if(!backend.Paths.ignoreModFolders.contains(folder))
				{
		...
		buttonToggle = new FlxButton(startX, 0, "ON", function()
		{
			...
			updateButtonToggle();
			FlxG.sound.play(backend.Paths.sound('scrollMenu'), 0.6);
		});
		...
		buttonToggle.label.setFormat(backend.Paths.uiFont, 24, FlxColor.WHITE, CENTER);
		...
		buttonUp.label.setFormat(backend.Paths.uiFont, 24, FlxColor.BLACK, CENTER);
		...
		buttonDown.label.setFormat(backend.Paths.uiFont, 24, FlxColor.BLACK, CENTER);
		...
		buttonTop.label.setFormat(backend.Paths.uiFont, 24, FlxColor.BLACK, CENTER);
		...
		buttonDisableAll.label.setFormat(backend.Paths.uiFont, 24, FlxColor.BLACK, CENTER);
		...
		buttonEnableAll.label.setFormat(backend.Paths.uiFont, 24, FlxColor.BLACK, CENTER);
		...
		installButton.label.setFormat(backend.Paths.uiFont, 16, FlxColor.WHITE, CENTER);
		...
		{
			var path = haxe.io.Path.join([backend.Paths.mods(), modsList[curSelected][0]]);
		...
		removeButton.label.setFormat(backend.Paths.uiFont, 16, FlxColor.WHITE, CENTER);
		...
		descriptionTxt.setFormat(backend.Paths.uiFont, 32, FlxColor.WHITE, LEFT);
		...
		while (i < modsList.length)
		{
			...
			if(!FileSystem.exists(backend.Paths.mods(values[0])))
			{
			...
			var iconToUse:String = backend.Paths.mods(values[0] + '/pack.png');
			...
			{
				newMod.icon.loadGraphic(backend.Paths.image('unknownMod'));
			}
...
class ModMetadata
{
	...
	public function new(folder:String)
	{
		...
		//Try loading json
		var path = backend.Paths.mods(folder + '/pack.json');
		if(FileSystem.exists(path)) {
