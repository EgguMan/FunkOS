...
class AtlasFrameMaker extends FlxFramesCollection
{
	...
	*/
	...
	public static function construct(key:String,?_excludeArray:Array<String> = null, ?noAntialiasing:Bool = false):FlxFramesCollection
	{
		...
		if (backend.Paths.fileExists('images/$key/spritemap1.json', TEXT))
		{
			trace("Only Spritemaps made with Adobe Animate 2018 are supported");
			return null;
		}

		var animationData:AnimationData = Json.parse(backend.Paths.getTextFromFile('images/$key/Animation.json'));
		var atlasData:AtlasData = Json.parse(backend.Paths.getTextFromFile('images/$key/spritemap.json').replace("\uFEFF", ""));

		var graphic:FlxGraphic = backend.Paths.image('$key/spritemap');
